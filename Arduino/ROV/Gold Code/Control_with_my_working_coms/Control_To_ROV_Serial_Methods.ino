...
ROV_RESPONSE rov_response = getNewROVResponseObject();
...
ROV_RESPONSE getNewROVResponseObject()
{
...
int secondAttemptReadCounter = 0;
char charRead = 0;
String rovMessage = "";
            ...
            {   // We have a valid marker for the beginning of a Response; Get the rest of it.
                ...
                while(stillReading)
                {
                    if( Serial1.available() > 0)
                    {
                        ...
                        if(charRead == EOM)
                        {
                            if(localDebug) LogMsg("Inside 'if(charRead == EOM)', setting stillReading to false");
                            stillReading = false;
                        }
                    }
                    else
                    {
                        if(localDebug) LogMsg("Inside second 'if(Serial1.available() > 0)...else' statement; doing delay()");
                        delay(DELAY_TIME_BETWEEN_ATTEMPTED_READS);
                        secondAttemptReadCounter++;
                        if(localDebug) LogMsg("Incremented secondAttemptReadCounter to " + String(secondAttemptReadCounter));
                        if(secondAttemptReadCounter > DELAY_TIME_BETWEEN_ATTEMPTED_READS)
                        {
                            if(localDebug) LogMsg("Inside second 'if(secondAttemptReadCounter == DELAY_TIME_BETWEEN_ATTEMPTED_READS); Setting 'stillReading = false'");
                            stillReading = false;
                        }
                    }
                }
